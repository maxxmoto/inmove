...
# ---------- НАСТРОЙКИ ----------
TOKEN = '8980504185:AAHXsZNvk-KPK-J8nmNxuJZbUfESlbSWzHM'
...
main_keyboard.row('🛵 Каталог', '🔍 Подобрать транспорт')
...
def category_keyboard():
    ...
    kb.add(
        types.InlineKeyboardButton('Scooter', callback_data='cat_scooter'),
        types.InlineKeyboardButton('Pitbike', callback_data='cat_pitbike'),
        types.InlineKeyboardButton('Moped', callback_data='cat_moped'),
        types.InlineKeyboardButton('Moto', callback_data='cat_moto'),
        types.InlineKeyboardButton('Ebike', callback_data='cat_ebike'),
        types.InlineKeyboardButton('Atv', callback_data='cat_atv'),
...
def resources_keyboard():
    ...
    kb.add(types.InlineKeyboardButton('📱 Telegram-канал', url='https://t.me/INMOVE812'))
    kb.add(types.InlineKeyboardButton('🛒 Авито', url='https://www.avito.ru/brands/inmove'))
    return kb
...
def handle_category(call):
    ...
    for moto in motos:
        ...
        markup.add(types.InlineKeyboardButton('📄 Подробнее на сайте', url=f'http://localhost:5000/moto/{moto["id"]}'))
    ...
    if 'каталог' in text:
        ...
        if user_code:
            ...
            bot.send_message(
                ...
                f'Ваш код: `{new_code}`\n\n'
                f'Отправьте его другу. Когда друг назовёт этот код менеджеру при покупке — вы получите скидку *2000₽* на следующую покупку или ТО!\n\n'
        ...
        if user_reminders:
            ...
            bot.send_message(
                message.chat.id,
                '⏰ *Напоминания*\n\n'
                'Когда смотрите товар в каталоге, нажмите «🔔 Напомнить» — и бот пришлёт вам напоминание.',
                parse_mode='Markdown'
            )
        ...
        bot.send_message(message.chat.id, 'Гарантия 14 дней. Подробности у менеджера.')
...
def handle_quiz(call):
    ...
    # Шаг 4: результат
        ...
        if terrain == 'city':
            filtered = [m for m in filtered if m['category'] in ['scooter', 'moped']]
        elif terrain == 'offroad':
            filtered = [m for m in filtered if m['category'] in ['pitbike', 'atv']]
        elif terrain == 'highway':
            filtered = [m for m in filtered if m['category'] in ['ebike', 'moped', 'moto']]
        elif terrain == 'anywhere':
            filtered = [m for m in filtered if m['category'] in ['ebike', 'pitbike']]
        ...
        if not filtered:
            msg_text = "Здравствуйте! Подбирал транспорт через бота, но ничего не подошло. Помогите с выбором."
            markup = types.InlineKeyboardMarkup()
            ...
            for moto in filtered[:3]:
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton('💬 Узнать цену', url=f'https://t.me/INMOVE812?text={encoded_text}'))
                markup.add(types.InlineKeyboardButton('📄 Подробнее', url=f'http://localhost:5000/moto/{moto["id"]}'))
...
def handle_remind(call):
    ...
    # Установка напоминания на конкретный товар
    ...
    if moto:
        ...
        def send_reminder():
            time.sleep(60)  # Для теста 1 минута
            try:
                markup = types.InlineKeyboardMarkup()
